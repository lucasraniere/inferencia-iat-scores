...
iat %>% 
    group_by(sex) %>% 
    summarise(media = mean(d_art), desv_padr = sd(d_art), N = n())
```
...
Em média, as mulheres que participaram do experimento tiveram uma associação implícita (medida pelo IAT) com a matemárica **negativa** e **média/forte** (média 0.481, desv. padrão 0.451, N = 121). Homens tiveram uma associação **negativa** com a matemática, portanto **menor** que a das mulheres (média 0.0965, desv. padrão 0.460, N = 20). Houve portanto uma **considerável** diferença entre homens e mulheres (diferença das médias -0.3848739, 95% CI [-0.161596, -0.592307]). A partir desta amostra, estimamos que...

mulheres têm uma associação negativa mais forte, porém não é claro se essa diferença é grande, moderada ou pequena. É necessário coletar mais dados para determinar se a diferença é relevante ou negligenciável, pois nesse estudo existe uma diferença entre o a quantidade de mulheres (121) e homens (20).
